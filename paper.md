...
As preparation for breaking the age-metallicity degeneracy with additional bands, a dedicated GALEX client (`StellarPop::Clients::GalexClient`) was added in the application `lib/` layer. The client performs MAST/GALEX GR6+7 cone-search requests, parses VOTable XML fields (`NUV_MAG`, `FUV_MAG`, errors, and object ID), computes on-sky angular separation, and returns the nearest in-radius UV match in a normalized hash format for downstream ingestion. Error handling is intentionally non-raising (`nil` on failure) with warning logs so asynchronous pipelines can degrade gracefully when external services are unavailable.
...
For GALEX UV ingestion readiness, the client layer is implemented and callable, but live runtime validation is currently limited by external endpoint reachability in the present environment (DNS resolution failures to `galex.stsci.edu` observed during development runs). This is treated as an operational connectivity limitation rather than a model-path failure.
