...
During the current refactor cycle, measurement persistence was split from galaxy identity into dedicated tables: `galaxy_photometries` and `galaxy_spectroscopies`. The `galaxies` table now serves primarily as target identity/provenance (`name`, `ra`, `dec`, object identifiers, catalog metadata), while SDSS ingestion tasks upsert photometric and spectroscopic measurements in their corresponding tables. For backward compatibility with existing jobs and reports, key measurement values are still mirrored into legacy `galaxies` columns as cache fields during transition.
