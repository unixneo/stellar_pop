namespace :galaxies do
  ...
  task backfill_measurement_tables: :environment do
    legacy_columns = %w[
      ...
      mag_type sdss_clean id_match_quality id_match_distance_arcsec id_match_note
      redshift_z z_err z_warning redshift_source redshift_confidence redshift_checked_at
    ]
    unless (legacy_columns - Galaxy.column_names).empty?
      puts "Legacy galaxy measurement columns already removed; backfill task skipped."
      next
    end
